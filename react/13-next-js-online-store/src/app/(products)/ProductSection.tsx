...
function ProductSection({title, viewAllButton, products, compact = false}: ProductsSectionProps) {
    return (
        <section>
            <div className={`flex flex-col ${!compact ? "px-[max(12px,calc((100%-1208px)/2))] mt-20" : ""}`}>
                ...
                <ul className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 xl:gap-10 justify-items-center">
                    {products.map((item, index) => (
                        <li
                            key={item._id}
                            className={compact ? `${index >= 4 ? "hidden" : ""} ${index >= 3 ? "md:hidden xl:block" : ""} ${index >= 4 ? "xl:hidden" : ""}` : ''}>
                            <ProductCard {...item} />
                        </li>
                    ))}
                </ul>
