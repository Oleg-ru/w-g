...
function ArticlesSection({title, viewAllButton, articles, compact = false}: ArticlesSectionProps) {
    return (
        <section>
            <div className={`flex flex-col text-[#414141] ${!compact ? "px-[max(12px,calc((100%-1208px)/2))] mt-20" : ""}`}>
                <div className="mb-4 md:mb-8 xl:mb-10 flex flex-row justify-between">
                    ...
                    <ViewAllButton btnText={viewAllButton.text}
                                   href={viewAllButton.href}
                    />
                </div>
                <ul className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-3 gap-6">
                    {articles.map((article, index) => (
                        <li key={article._id} className={`h-75 md:h-105 ${compact ? `${index >= 4 ? "hidden" : ""} ${index >= 3 ? "md:hidden xl:block" : ""} ${index >= 4 ? "xl:hidden" : ""}` : ''}`}>
